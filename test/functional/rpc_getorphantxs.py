...
class GetOrphanTxsTest(BitcoinTestFramework):
    ...
    def test_orphan_activity(self):
        ...
        assert_equal(len(node.getorphantxs()), 2)
        assert node.tx_in_orphanage(tx_child_1["tx"])
        assert node.tx_in_orphanage(tx_child_2["tx"])
    ...
    def test_orphan_details(self):
        ...
        assert not node.tx_in_orphanage(tx_child_2["tx"])
