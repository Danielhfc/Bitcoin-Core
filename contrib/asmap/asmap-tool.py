...
def main():
    ...
    asmap_tool_dir = Path(__file__).resolve().parent
    default_infile = asmap_tool_dir / "ip_asn.dat"
    ...
    project_dir = asmap_tool_dir.parents[1]
    default_outfile = project_dir / "src" / "init" / "ip_asn.h"
    ...
    if args.subcommand is None:
        ...
        file_content = f"""#ifndef BITCOIN_INIT_IP_ASN_H
...
/**
 * ASMap data, mapping IP prefixes to ASNs,
 * AUTOGENERATED by contrib/asmap/asmap-tool.py
 */

inline constexpr std::array<std::byte, {len(data)}> ip_asn = {{
{hex_data}
}};
